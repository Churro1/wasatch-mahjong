-- 20260728193000_fix_finalize_checkout_coupon_upsert_ambiguity.sql
-- Avoids PL/pgSQL name ambiguity around order_id in coupon_uses upsert logic.
...
begin
  ...
  if v_coupon_code <> '' and coalesce(p_coupon_discount_amount, 0) > 0 then
    ...
    if found then
      update public.coupon_uses as uses
      set discount_amount_cents = p_coupon_discount_amount
      where uses.coupon_id = v_coupon.id
        and uses.order_id = v_order.id;
